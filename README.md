...
uv run donazopy export cloudflare/example.com --output=example.com.zone --overwrite
uv run donazopy export cloudflare/* --output=zones --dotenv-path=.env   # writes zones/<domain>.zone for every zone --skip-ns

# Import BIND zone text into a provider
uv run donazopy import-zone cloudflare/example.com example.com.zone --dotenv-path=.env
uv run donazopy import-zone cloudflare/example.com example.com.zone --clean --dotenv-path=.env   # wipe target first
...
uv run donazopy copy cloudflare/* ionos/ --clean --skip-ns --dotenv-path=.env   # iterate every Cloudflare zone, copy each to IONOS (same name)
...
| `donazopy export TARGET [--output=PATH] [--overwrite] [--skip-ns] [--skip-types=A,AAAA,...] [--dotenv-path=PATH]` | Export zone as BIND text. Wildcard target (`provider/*`) requires `--output=DIR` and writes each zone to `DIR/<domain>.zone`; for single-domain targets `--output` may be a file or a directory (`DIR/<domain>.zone` is autogenerated). |
| `donazopy import-zone TARGET PATH [--proxied] [--clean] [--dotenv-path=PATH]` | Import BIND zone file into provider; `--clean` wipes existing records first |
...
| `donazopy copy SOURCE DEST [--skip-ns] [--skip-types=...] [--replace] [--clean] [--create=BOOL] [--dotenv-path=PATH]` | Copy zone between provider targets; by default the destination zone is created if missing (`--create=False` to skip). `--clean` is a synonym for `--replace` (wipe target before copying). Wildcard source (`provider/*`) iterates every zone the source provider manages and copies each to the same-named zone on the destination (use `dest_provider/`). |
