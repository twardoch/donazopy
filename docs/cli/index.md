...
Exports the `SOURCE` zone and imports it into the `DEST` zone — a convenience for migrating or cloning zones (typically within the same provider).
...
Passing `NS1 NS2 ...` to *set* registrar-level nameservers is **not** supported for Cloudflare and is out of scope today: changing a domain's delegation is a registrar/parent-zone operation, not a hosted-zone one. See [Providers → Nameservers and delegation](https://github.com/twardoch/donazopy/providers/#nameservers-and-delegation).
