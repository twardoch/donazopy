...
def test_credential_status_when_dotenv_has_token_then_redacts_value(tmp_path: Path) -> None:
    ...
    dotenv_path.write_text("CLOUDFLARE_DNS_TOKEN=dotenv-token\n", encoding="utf-8")

    status = credential_status(provider, dotenv_path=dotenv_path, env={}, discover=False)

    assert status.is_complete is True
    assert status.present == ("CLOUDFLARE_DNS_TOKEN",)
    assert status.missing == ()
    assert status.to_dict()["redacted"] == {"CLOUDFLARE_DNS_TOKEN": "***"}
    assert status.sources["CLOUDFLARE_DNS_TOKEN"] == str(dotenv_path)
...
def test_load_provider_credentials_when_env_and_dotenv_have_token_then_env_wins(tmp_path: Path) -> None:
    ...
    credentials = load_provider_credentials(
        ...
        env={"CLOUDFLARE_DNS_TOKEN": "environment-token"},
        discover=False,
    )

    assert credentials == {"CLOUDFLARE_DNS_TOKEN": "environment-token"}
...
def test_credential_status_when_token_missing_then_reports_missing() -> None:
    ...
    assert status.missing == ("CLOUDFLARE_DNS_TOKEN",)
...
def test_provider_status_when_credentials_required_then_raise_clear_error() -> None:
    ...
    with pytest.raises(
        ProviderCredentialError, match="missing required credentials for cloudflare: CLOUDFLARE_DNS_TOKEN"
    ):
